...
sgdisk -t ${PARTITION_HOME}:8300 ${INSTALL_DRIVE}
...
sgdisk -c ${PARTITION_HOME}:"${LABEL_HOME}" ${INSTALL_DRIVE}
...
mount ${INSTALL_DRIVE}${PARTITION_HOME} ${MOUNT_PATH}
...
/dev/disk/by-partlabel/${LABEL_HOME}				/      		ext4	rw,relatime,data=ordered,discard	0	2
FSTAB_EOF
}
